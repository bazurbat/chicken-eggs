...
(define parse-type-rec
  (match-lambda
    ...
    [`(size_t . ,more) (values 'size_t more)]
...
(define (c-exception-wrapper name argtypes safe rtype)
  (if c-exception-handler
      (let ((vars (map (lambda _ (gensym "a")) argtypes)))
        ;; TODO: convert this to cexp and apply foreign-transform
...
(define (process-struct-member-def m sname name type mut?)
  (let ([getter (fix-name (string-append (->string sname) "-" (->string name)))])
    (let ((g (foreign-transformer
              `(,(rename 'foreign-lambda*) ,type (((c-pointer (,m ,sname)) s))
                (-> s ,name) ) rename) )
	  (s (foreign-transformer
              `(,(rename 'foreign-lambda*) void (((c-pointer (,m ,sname)) s)
                                                 (,type x) )
                (= (-> s ,name) x) )
              rename) ) )
      (emit
  ...
  (let loop ((opts opts))
    (match opts
	 ...
	 (append name-substitution-repls (list usual-naming-transform)))
      ...
      (('foreign-transformer: f . more)
       ;; TODO: is eval here good or bad? f is a symbol for a
       ;; user-defined function definition. we want that function, not
       ;; its name.
       ;; FW: this may possibly require "##sys#eval/meta"
       (set! foreign-transformer (eval f))
       (loop more))
